...
local function charbaseUpdate(frame, msg)
    ...
    -- LEVEL_UPDATE is ON_INIT or base level up.
    if (settings.xpbase.now == 0 and newxp > 0) then
...
local function charjobUpdate(frame, msg, str, newxp, tableinfo) 
    ...
    -- for msg: JOB_EXP_UPDATE (usual msg: JOB_EXP_ADD)
	if (settings.xpjob.now == 0 and curexp > 0) then
		settings.xpjob.now = curexp;
		return;
	end

	if (not options.show.xpjob) then 
        return;
    end
    ...
    local max = tableinfo.endExp - tableinfo.startExp;
	local prgain = settings.xpjob.gain / max * 100;
